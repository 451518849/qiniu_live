package com.xiaofa.qiniu_live.utils;
...
import com.xiaofa.qiniu_live.model.UpdateInfo;
import com.xiaofa.qiniu_live.model.UserList;
...
public class QNAppServer {
    ...
    public UserList getUserList(String context, String roomName) {
        String url = APP_SERVER_ADDR + "/v1/rtc/users/app/" + getAppId() + "/room/" + roomName;
        try {
            OkHttpClient okHttpClient = new OkHttpClient.Builder().sslSocketFactory(new SSLSocketFactoryCompat(), getTrustManager()).build();
            Request request = new Request.Builder()
                    .url(url)
